...
Apple stores the advertisement key together with its corresponding location report on their servers. From this, Apple can infer how many trackers are in use and estimate the general region in which they are being used, based on the uploader's IP address.
\bigskip
...
\\
\\
Additionally, if Apple is considered an untrustworthy entity, the reliance on static keys increases the linkability risk. By analyzing the metadata accompanying location reports, such as the static advertisement key and the finding device's IP address, Apple could infer the user's approximate location and reconstruct movement patterns or daily routines.
\\
\begin{itemize}
...
\begin{table}[H]
    \centering
    \caption{GDPR Stakeholder Analysis}
    ...
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \textbf{Lawfulness (Art 6):} Circumvents anti-stalking protections; risks of non-consensual surveillance. \\ \hline
        ...
        \textbf{Integrity \& Confidentiality:} Critical failures (plaintext credentials, unencrypted HTTP). Data becomes personal if linked to a specific person.\newline
        ...
        \textbf{Mitigation:} Rolling keys significantly reduce the ability to link reports to a single device. \\ \hline
    \end{tabularx}
\end{table}
...
Static advertisement keys allow anyone to track a device by listening for its BLE signal. To fix this, we recommend using rolling codes, which means the key changes periodically (for example, every 24 hours). This prevents strangers from linking different packets to the same device, but the owner can still find it because they have the secret key to calculate the list of all past and future advertisement keys.\\
...
\hfill
\begin{minipage}[t]{0.48\linewidth}
\begin{lstlisting}[style=cstyle, caption={Rolling Key Test Output}, basicstyle=\ttfamily\tiny\color{codetext}]
Unity test run 1 of 1
Macless Haystack Test - Non-Rolling Code Privacy Test Suite
=============================================================
...
Our recommended solution to the GDPR and EULA violations is to obtain explicit consent from the user regarding the use of the application and the collection of their data. Additionally, it is crucial to inform users that the application is used at their own risk, and that they are advised to host the Macless-Haystack server themselves while being fully aware of the privacy implications associated with this application.

A similar disclaimer is included in the GitHub README; however, this information should also be incorporated directly into the application itself, together with a clear consent form.
\newpage
\section{Conclusion}
