...
At a high level, the trackers broadcast an identifier which can be detected by nearby Apple devices. These Apple devices then anonymously upload an encrypted report which contains its current GPS location. The owner can later retrieve the location of their tracker by requesting these reports.
...
As config.ini is not written to by the program, it cannot be unit tested in the traditional sense. To ensure privacy, we added a specific test to verify that the file does not contain any plaintext passwords.
The key generation script's procedural style, standard unit testing is not possible. We tested it by mocking the environment and executing the script string directly, though a object oriented rewrite would be a more robust solution.\\    
...
class TestHTTPSConfiguration:
    ...
    def test_runs_without_certificate(self, mock_isfile, mock_check_anisette, mock_http_server):
...
\hfill
\begin{minipage}[t]{0.49\linewidth}
\begin{lstlisting}[style=cstyle, caption={Test output for HTTPS Configuration}, label={lst:https_test_output}, basicstyle=\ttfamily\tiny\color{codetext}, keywords={FAILED, AssertionError, Failed}, keywordstyle=\color{red}\bfseries, moredelim={[l][\color{red}]{E\ \ }}]
===================== test session starts =====================
platform linux -- Python 3.10.19, pytest-8.4.2, pluggy-1.6.0 -- /home/cyuzuzo/.pyenv/versions/3.10.19/bin/python3.10
cachedir: .pytest_cache
rootdir: /home/cyuzuzo/Projects/macless-haystack
...
\begin{table}[H]
    ...
    \begin{tabularx}{\textwidth}{|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}X|>{\raggedright\arraybackslash}X|}
        ...
        \textbf{Lawfulness (Art 6):} Can more easily circumvents anti-stalking protections making non-consensual surveillance possible.\newline \textit{Household Exemption: Generally, for purely personal/household activities, GDPR may not apply (Art 2(2)(c)).} \newline
 \\ \hline
                \textit{Household Exemption: Generally, for purely personal/household activities, GDPR may not apply (Art 2(2)(c)).} \newline
...
The following code snippets show how this can be implemented, in both cases a json load / plain text read has been replaced by a keyring.get_password call.

\begin{lstlisting}[style=cstyle, language=Python, caption={Secure implementation for appleid retrieval}, label={lst:secure_config}, basicstyle=\ttfamily\tiny\color{codetext}]
def getUser():
...
\end{lstlisting}
To solve the issue that the private-public keypairs are stored in plain text, a similar approuch can be used but using the flutter secure storage library.
This will require a reworking of the generate\_keys.py script as its currently written in python, the reason its best to port it first to flutter is that sharing credentials saved in secure storage is non trivial and porting the short key generation script to flutter should be a small task.
...
An small assertion can be added to the server to check if the certificate is there, if not, the server should exit with an error message. This prevents the server from running without a certificate.
