...
\begin{figure}[ht]
    \centering
    \resizebox{\textwidth}{!}{\input{architecture}}
...
p_i &= d_i * G
\end{align}
This requires a small firmware update. The tracker needs to store the master keys and a function to calculate the new ones. This is easy to do on chips like the ESP32 or nrf5x. We tested this and confirmed the tracker can generate new keys every 15 minutes. It should also save the current key to memory so it doesn't lose its place if the battery dies.

\subsubsection{Client-Side Challenges}
Updating the user application is harder because custom trackers are not as reliable as official AirTags. Official tags are always on, DIY trackers might be turned off for days or weeks. This causes a synchronization problem: when the tracker is off, it resumes from where it stopped, thinking it is an earlier time than it really is. It broadcasts a key that belongs to the past.
To find the tracker, the application must ask Apple for reports matching a list of possible keys. Normal Apple devices might check a 7-day window. We cannot do this because checking 7 days worth of keys (over 600 requests) is too much. It would likely get the user's account banned by Apple for making too many requests. It also makes the app slow.
We recommend limiting the rolling rate to 24 hours, this way we can check 7 days worth of keys in 7 requests. Because of the sync issue, if the tracker has been off, the app effectively needs to search through "old" keys to find where the tracker left off. This means finding a device that has been desynchronized for a long time might be slow or difficult to avoid hitting Apple's limits.
